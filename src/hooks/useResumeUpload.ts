...
import { createResumeRecord } from '../services/resumeService';
import { sendToMakeWebhook } from '../services/resumeService';
	...
	const uploadResume = async (file: File, jobUrl: string) => {
		try {
			...
			try {
				...
				console.error('Failed to send to Make webhook:', err);
			}
